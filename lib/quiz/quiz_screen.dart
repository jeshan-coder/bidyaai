import 'package:anticipatorygpt/chat/chat_bloc.dart';
...
class QuizScreen extends StatelessWidget {
  ...
  const QuizScreen({super.key,required this.quiz,required this.chatInstance});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocProvider(
          create: (context)=>QuizBloc(chat: chatInstance)..add(InitializeQuiz(quiz: quiz)),
          child: BlocConsumer<QuizBloc,QuizState>(listener:(context,state){
          if(state is QuizError)
            {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text("Quiz Error: ${state.error}"),
            ...
            itemBuilder:(context,index){
              ...
              final isExplanationLoading= state is QuizLoadingExplanation && state.questionIndexLoading==index;
              ...
              return QuizQuestionCard(
                ...
                onOptionSelected: (optionIndex){
                  context.read<QuizBloc>().add(
                    SubmitAnswer(questionIndex: index, selectedOptionIndex:optionIndex)
                  );
                },
                onRequestExplanation: (){
                  context.read<QuizBloc>().add(
                    RequestExplanation(questionIndex: index,
                        questionText: question.questionText, options: question.options, selectedOptionIndex:selectedAnswer!)
                  );
                },
              );
            });
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: Column(
        ...
        children: [
          Text('Question ${questionIndex +1}: ${question.questionText}',
          style: TextStyle(
            fontSize: 18,
          ...
          ...question.options.asMap().entries.map((entry){
            ...
            child: ChoiceChip(
              ...
              labelStyle: TextStyle(
                color: hasAnswered && isSelected?theme.colorScheme.onPrimary:theme.colorScheme.onSurface,
          ...
          if(hasAnswered)...[
            ...
            Align(
              ...
              child:ElevatedButton.icon(onPressed:isExplanationLoading?null:onRequestExplanation ,icon: isExplanationLoading?const SizedBox(width: 16,height: 16,child: CircularProgressIndicator(strokeWidth: 2,),):const Icon(Icons.lightbulb_outline)
          ...
          if(explanation!=null && !isExplanationLoading)...[
            ...
            Text(explanation!,
              style:TextStyle(fontSize: 14,color:Color.fromARGB(
