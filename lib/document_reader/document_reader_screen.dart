...
/*
Screen for reading document along with ai
_captureScreenshot : captures a screenshot of the current PDF page.
_sendMessageWithScreenshot : sends the message and screenshot to the AI.
*/

// Pass InferenceChat and languageCode to the screen.
class DocumentReaderScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      // Create the bloc with the chat instance and language code.
      create: (_) =>
          DocumentReaderBloc(chat: chatInstance, languageCode: languageCode),
      child: _DocumentView(),
    );
  }
}
...
class _DocumentViewState extends State<_DocumentView> {
  ...
  // GlobalKey to capture the screenshot of the PDF viewer.
  ...
  // Captures a screenshot of the current PDF page.
  ...
  // Sends the message and screenshot to the AI.
  Future<void> _sendMessageWithScreenshot() async {
    final text = _textController.text.trim();
    if (text.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Please type a question.')));
      return;
    }

    final pdfState = context.read<DocumentReaderBloc>().state;
    if (pdfState is! DocumentLoaded &&
        pdfState is! DocumentLoadedWithResponse) {
    ...
    if (imageBytes != null) {
      context.read<DocumentReaderBloc>().add(
        ProcessDocument(message: text, imageBytes: imageBytes),
      );
      ...
      body: BlocConsumer<DocumentReaderBloc, DocumentState>(
        ...
        builder: (context, state) {
          ...
          if (state is DocumentLoaded ||
              ...
              state is DocumentLoadedWithResponse) {
            final pdfBytes = state is DocumentLoaded
                ? state.pdfBytes
                : state is DocumentAnalyzing
                ? state.pdfBytes
  ...
  // This widget is now more complex to include AI interaction.
              ...
              RepaintBoundary(
                ...
                child: SfPdfViewer.memory(
                  ...
                  onDocumentLoaded: (details) {
                    ...
                    context.read<DocumentReaderBloc>().add(
                      DocumentPageLoaded(total),
                    );
                  },
                  onPageChanged: (details) {
                    context.read<DocumentReaderBloc>().add(
                      DocumentPageChanged(details.newPageNumber),
                    );
                  },
                ),
              ),
              // Made the AI response container scrollable.
              if (state is DocumentAnalyzing ||
                  state is DocumentLoadedWithResponse)
                Align(
                  ...
                  child: Container(
                    ...
                    constraints: BoxConstraints(
                      maxHeight:
                          ...
                          0.4, // Constrain height to prevent overflow
                    ),
                    child: SingleChildScrollView(
                      child: Text(
                        state is DocumentAnalyzing
                            ? state.currentResponse
                            : (state as DocumentLoadedWithResponse).aiResponse,
