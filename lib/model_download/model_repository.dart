...
class ModelRepository {
  ...
  static const String _huggingFaceToken = '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';
  ...
  Future<bool> checkIfModelExists() async {
    ...
    if (localFileLength < _minExpectedSizeBytes) {
      print("Local file is too small. It's likely corrupt or not the real model. Deleting it.");
      await file.delete();
      return false;
    }
  ...
  Future<String> downloadModel(Function(double) onProgress) async {
    ...
    try {
      ...
      if (e.response?.statusCode == 401) {
        print("Authorization failed. Please check your Hugging Face token in model_repository.dart.");
        throw Exception("Authorization failed (401). Is your Hugging Face token correct?");
      }
      ...
      throw Exception("Failed to download model. Please check your network connection and try again.");
    }
  }
}
