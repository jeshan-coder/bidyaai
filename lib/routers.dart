...
/*
  defines app route to all screen.
  provides bloc provider to all screens.
 */
class AppRoutes {
    ...
    switch (settings.name) {
      ...
      case AppRoutes.quiz:
        ...
        final InferenceChat chatInstance =
            args['chatInstance'] as InferenceChat; // InferenceChat instance
        ...
        if (chatInstance == null) {
          ...
          return MaterialPageRoute(
            builder: (_) => const Center(
              child: Text('Error:AI Model not available for quiz.'),
            ),
          );
        }
      ...
      case AppRoutes.cameraLive:
        final args = settings.arguments as Map<String, dynamic>;
        final InferenceChat chatInstance =
            args['chatInstance'] as InferenceChat;

        if (chatInstance == null) {
          return MaterialPageRoute(
            builder: (_) => const Center(
              child: Text("Error: chat instance not available for camera AI"),
            ),
          );
        }
        return MaterialPageRoute(
          builder: (_) => BlocProvider(
            create: (context) => CameraBloc(chat: chatInstance),
      ...
      case AppRoutes.documentReader:
        ...
        if (chatInstance == null) {
          return MaterialPageRoute(
            builder: (_) => const Center(
              child: Text("Error:Chat instance not available for document"),
            ),
          );
        }
        ...
        return MaterialPageRoute(
          builder: (_) => BlocProvider(
            create: (context) => DocumentReaderBloc(
              chat: chatInstance,
              languageCode: languageCode,
            ),
            child: DocumentReaderScreen(
